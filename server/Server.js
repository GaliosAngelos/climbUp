...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "Anmeldefehler. Bitte versuchen Sie es erneut." });
  }
});
...
app.post("/query", async (req, res) => {
  if (!dbClient) {
    return res
      .status(401)
      .send({ message: "Nicht angemeldet. Bitte melden Sie sich erneut an." });
  }
  try {
    ...
    res
      ...
      .send({
        message:
          "Fehler bei der Ausführung der Datenbankabfrage. Bitte melden Sie sich erneut an.",
      });
  }
});
...
app.post("/logout", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: "Database connection successfully closed" });
  } catch (error) {
    ...
    res
      .status(500)
      .send({ message: "Error while closing database connection" });
  }
});
